...
void enqueue(struct proc* rp)
{
	...
	if ((Scheduling == Default)) { // || !isuserp(rp)){
		...
		rp->p_quantum_size = QuantimSize;
		if (rdy_head[User_Q_level] == NIL_PROC) {
			// printf("%s if\n",rp->name);
			rp->p_ticks_left = QuantimSize;
			...
			rp->p_nextready = rdy_head[User_Q_level];		/* chain head of queue */
...
int main() {
	...
	for (int i = 0; i < 10 + 20 + 5 + 3; i++) {
		...
		if (next_ptr != NIL_PROC) {
			printf("cycle %d : %s is now running with remaing time = %d and quantum_left=%d \n", i + 1, next_ptr->name, next_ptr->process_time, next_ptr->p_ticks_left);
		}
