 ...
 */
  ...
  if ( (dst_ptr->p_rts_flags & (RECEIVING | SENDING)) == RECEIVING &&
	...
	CopyMess(caller_ptr->p_nr, caller_ptr, m_ptr, dst_ptr,
		 dst_ptr->p_messbuf);
	if ((dst_ptr->p_rts_flags &= ~RECEIVING) == 0) enqueue(dst_ptr); /* if process was blocked waiting for the message add it back to the ready queue */
 ...
 *===========================================================================*/
// rp, User_Q_level
PRIVATE void ShortestJobFisrtSched(rp, char user_level )
register struct proc* rp;
{
    ...
    while (process != NIL_PROC) {
        if (rp->process_time < process->process_time) {
            placed = 1;
            if (process == rdy_head[user_level] && (process == next_ptr || process == proc_ptr)) {
...
PRIVATE void RoundRobinSched(rp,char user_level ,char quantum_size);
register struct proc* rp;
{
    rp->p_priority = user_level;
...
PRIVATE void PriorityBasedSched(rp);
register struct proc* rp;
{
    ...
    char placed = 0;
    while (process != NIL_PROC) {
        if (rp->u_priority < process->u_priority) {
            placed = 1;
 ...
 */
    ...
    //////////////////////////////
        ...
        ShortestJobFisrtSched(rp, User_Q_level);
    }
    else if (Scheduling == RoundRobin) {
        RoundRobinSched(rp, User_Q_level, QuantimSize);
    }
    else if (Scheduling == PriorityBased) {
        PriorityBasedSched(rp);
    }
 ...
 */
  static struct proc *prev_ptr = NIL_PROC;	/* previous without time , static as it will be called at first time only */ 
  ...
  if ( ! time_left) {				/* quantum consumed ? */
      ...
      if (prev_ptr == rp) penalty ++;		/* catch infinite loops , the process took two quantum after each other */
