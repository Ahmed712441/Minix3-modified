 ...
 *				alloc_bit				     *
 *===========================================================================*/
PUBLIC bit_t alloc_bit_old(sp, map, origin)
...
/* Allocate a bit from a bit map and return its bit number. */
  ...
  if (map == IMAP) { /* Search i-Node bitmap*/
	start_block = START_BLOCK;
	map_bits = sp->s_ninodes + 1;
	bit_blocks = sp->s_imap_blocks;
  } else {/* Search Zone bitmap*/
  ...
  do {
	...
	for (wptr = &bp->b_bitmap[word]; wptr < wlim; wptr++) {
		...
		/* Does this word contain a free bit? */ /*if the current word is equal to the one's complement of zero?*/
		if (*wptr == (bitchunk_t) ~0) continue;

		/* Find and allocate the free bit. */ /* find the free bit in the word*/
	...
	put_block(bp, MAP_BLOCK);
	/* this happens when the origin is set */
...
bit_t origin;			/* number of bit to start searching at */
{
	...
	bit_t i, b;
	int allocated = 0;
	bit_t allocated_bits[ALLOCATION_BITS];
	unsigned allocated_blocks[ALLOCATION_BITS];
	...
	do {
		...
		for (wptr = &bp->b_bitmap[word]; wptr < wlim; wptr++) {
			...
			*wptr = conv2(sp->s_native, (int)k);
			bp->b_dirt = DIRTY;
			put_block(bp, MAP_BLOCK);
			allocated_bits[allocated] = b;
			allocated_blocks[allocated] = block;
			allocated++;
			if (allocated == ALLOCATION_BITS) {
	...
	for (int i = 0; i < ALLOCATION_BITS; i++) {
		bp = get_block(sp->s_dev, start_block + allocated_blocks[i] , NORMAL);
		bp->b_dirt = CLEAN;
		put_block(bp, MAP_BLOCK);
	}
	return(NO_BIT);		/* no bit could be allocated */
